...
void FileReader::SaveArrayToFile(vector<vector<float>> vector, string fileName) {
	ofstream file(fileName, ofstream::app);
	if (file.is_open()) {
		for (unsigned i = 0; i < vector.size(); i++) {
			for (unsigned j = 0; j < vector.at(i).size(); j++) {
...
bool FileReader::CheckExistFile(string fileName) {
	ifstream f(fileName);
	if (f.good()) {
		f.close();
		return true;
